...
<form class="form-horizontal" method="post" action="proses-pelatihan-datatesting.php">
	  <div class="form-group ">
		<label class="col-sm-6 control-label">Nilai K</label>
		<div class="col-sm-2 ">
...
<table class="table table-hover">
	...
	//include("config.php");
				$sql="SELECT ts.id, ts.id_merk,ts.id_type, ts.dp, ts.kontribusi, ts.nama_grade, td.nama_dealer, tt.nama_type, tm.nama_merk FROM data_training ts, dealer td, type tt, merk tm where ts.id_dealer=td.id_dealer && ts.id_type=tt.id_type && ts.id_merk=tm.id_merk order by id asc";
			...
			<td>
				<a href="edit_training.php?id=<?php echo $data['id']; ?>" class="btn btn-warning">Edit</a>
				<a onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='hapus_training.php?id=<?php echo $data['id']; ?>' }" class="btn btn-danger">Hapus</a>
			</td>
		</tr>		
		<?php 
	}
